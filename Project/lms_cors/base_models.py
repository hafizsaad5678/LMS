...
class BaseProfile(models.Model):
    ...
    def create_user_account(self, password=None):
        ...
        try:
            ...
            if existing_user:
                # Use update() to avoid calling save() which may call this method again
                self.__class__.objects.filter(pk=self.pk).update(user=existing_user)
                self.user = existing_user  # Update instance attribute
            ...
            if not password:
                # Generate initial password (first 6 chars of email + last 2 of phone)
                phone_part = self.phone[-2:] if self.phone else "00"
                password = f"{self.email[:6]}{phone_part}"
            ...
            self.user = user  # Update instance attribute
            return True, user
