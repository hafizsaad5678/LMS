<template>
  <StudentPageTemplate :title="studentName ? `Class Schedule: ${studentName}` : 'Class Schedule'"
    ...
    <div class="class-schedule">
      ...
      <!-- Ended Semester Warning State -->
      <div v-if="currentSemesterStatus && currentSemesterStatus !== 'active'" class="text-center py-5 px-4 bg-light rounded-4 shadow-sm border border-2 border-warning-subtle mx-auto my-5" style="max-width: 600px;">
        <div class="day-icon-wrapper size-64 rounded-circle bg-warning bg-opacity-10 shadow-sm d-flex align-items-center justify-content-center mx-auto mb-3" style="width: 64px; height: 64px;">
          <i class="bi bi-calendar-x fs-2 text-warning"></i>
        </div>
        <h4 class="fw-bold text-dark mb-2">Semester Schedule Ended</h4>
        <p class="text-muted small mx-auto mb-4" style="max-width: 500px;">
          Your current semester is marked as <span class="badge bg-student text-white fw-bold px-2 py-1">{{ currentSemesterStatus.toUpperCase() }}</span>. Weekly class timetables are only visible during active, ongoing academic terms.
        </p>
        <div class="mt-4">
          <router-link to="/student-dashboard" class="btn btn-student px-4 py-2 fw-semibold rounded-3 shadow-sm text-white">
      ...
      <template v-else>
        ...
        <!-- Empty State -->
        <div v-else-if="weekSchedule.length === 0" class="text-center py-5 bg-white rounded-4 shadow-sm border border-2 border-dashed border-light-subtle my-3">
          <i class="bi bi-calendar-x display-4 text-muted"></i>
          <h5 class="mt-3 fw-bold">No Classes Scheduled</h5>
          <p class="text-muted small mb-0">No active academic days in this week</p>
        </div>
          ...
          <div v-for="day in prioritizedWeekSchedule" :key="day.date" class="col-12 col-xl-6">
            <div class="day-card card border-0 shadow-sm rounded-4 overflow-hidden" :class="isToday(day.date) ? 'border-top border-student border-4' : ''">
              ...
              <div v-if="isDayExpanded(day)" class="card-body p-4">
                ...
                <div class="class-timeline" v-else>
...
const { studentName, studentEnrollment, currentSemesterStatus, currentSemesterStartDate, currentSemesterEndDate, loadProfile } = useStudentBase()
...
const weekSchedule = computed(() => {
  ...
  for (let i = 0; i < 7; i++) {
    ...
    // Formatted current day's date string in YYYY-MM-DD
    const yyyy = date.getFullYear()
    const mm = String(date.getMonth() + 1).padStart(2, '0')
    const dd = String(date.getDate()).padStart(2, '0')
    const dayDateStr = `${yyyy}-${mm}-${dd}`

    // Skip day if it is outside current semester date bounds!
    if (currentSemesterStartDate.value && dayDateStr < currentSemesterStartDate.value) {
      continue
    }
    if (currentSemesterEndDate.value && dayDateStr > currentSemesterEndDate.value) {
      continue
    }

    const dayClasses = schedule.value.filter(cls => {
      // 1. Day of the week check
      if (cls.day?.toLowerCase() !== dayName) return false

      // 2. Bound checks against semester start and end dates
      if (cls.semester_start_date && dayDateStr < cls.semester_start_date) return false
      if (cls.semester_end_date && dayDateStr > cls.semester_end_date) return false
...
const classesThisWeek = computed(() => weekSchedule.value.flatMap(d => d.classes))
const weekClassCount = computed(() => classesThisWeek.value.length)

const totalClasses = computed(() => classesThisWeek.value.length)
const uniqueSubjects = computed(() => [...new Set(classesThisWeek.value.map(c => c.subject_name || c.subject?.name).filter(Boolean))].length)
const uniqueInstructors = computed(() => [...new Set(classesThisWeek.value.map(c => c.instructor_name || c.teacher_name || c.teacher?.full_name).filter(Boolean))].length)
const totalHours = computed(() => {
  let total = 0
  classesThisWeek.value.forEach(c => {
...
const weekRange = computed(() => {
  if (weekSchedule.value.length === 0) return 'Semester Inactive / Out of Bounds'
  const first = weekSchedule.value[0].date
  const last = weekSchedule.value[weekSchedule.value.length - 1].date
  return `${formatDate(first)} - ${formatDate(last)}`
})

const currentMonthYear = computed(() => {
  if (weekSchedule.value.length === 0) {
